...
class CIFAR10_LeNet_Autoencoder(BaseNet):
    ...
    def forward(self, x, debug=False):
        ...
        if debug: print(f"After interpolate1: {x.shape}")
        x = self.deconv2(x)
        if debug: print(f"After deconv2: {x.shape}")
        x = F.interpolate(F.leaky_relu(self.bn2d5(x)), scale_factor=2)
        if debug: print(f"After interpolate2: {x.shape}")
        x = self.deconv3(x)
        if debug: print(f"After deconv3: {x.shape}")
        x = F.interpolate(F.leaky_relu(self.bn2d6(x)), scale_factor=2)
        if debug: print(f"After interpolate3: {x.shape}")
        x = self.deconv4(x)
        if debug: print(f"After deconv4: {x.shape}")
        x = torch.sigmoid(x)
